...
const UserList = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribePremiumStatuses = onSnapshot(
      collection(db, "premiumStatuses"),
      (snapshot) => {
        const statuses = {};
  ...
  return (
    <div className="p-5 bg-white border shadow-md shadow-black border-gray-300 rounded-lg">
      ...
      <ReactPaginate
        ...
        previousClassName={
          "px-3 py-1 text-confirm hover:opacity-60 duration-300 rounded-md cursor-pointer"
        }
        nextClassName={
          "px-3 py-1 text-confirm hover:opacity-60 duration-300 rounded-md cursor-pointer"
        }

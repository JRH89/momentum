      ...
      <div className='py-3'>
        <div className='px-5 md:px-10'>
          <div className='flex items-center justify-between'>
            ...
            <p className='text-2xl md:text-3xl font-bold'>{siteMetadata.title}</p>
           </div>
           
            <div className="md:hidden cursor-pointer hover:text-destructive duration-300" onClick={toggleMenu}>
              <MenuIcon className="h-10 w-10 " />
            </div>
            {/* Desktop Menu */}
            <nav className='hidden font-bold md:flex gap-6 text-black/80 items-center'>
              {pageData.menuItems.map((item, index) => (
                <Link key={index} href={item.href} className={item.isPrimary ? 'bg-destructive shadow-md font-bold shadow-black hover:shadow-lg hover:shadow-black duration-300 text-black px-4 py-2 rounded-lg inline-flex tracking-tight' : 'hover:pb-2 duration-300'}>
                  {item.text}
                </Link>
              ))}
            </nav>
          ...
          >
            {pageData.menuItems.map((item, index) => (
              <Link key={index} href={item.href} className={item.isPrimary ? 'bg-destructive text-black px-4 py-2 rounded-lg font-bold inline-flex tracking-tight shadow-md shadow-black hover:shadow-lg hover:shadow-black duration-300' : 'hover:pr-2 duration-300'} onClick={toggleMenu}>
                {item.text}
              </Link>
            ))}
          </motion.nav>
